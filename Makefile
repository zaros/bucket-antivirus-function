...
archive: clean  ## Create the archive for AWS lambda
	...
	docker buildx build --platform linux/amd64 --load -t bucket-antivirus-function:latest .
	docker run --platform linux/amd64 -v $(current_dir)/bin:/opt/mount --rm bucket-antivirus-function:latest  bash -c "cp -r ${container_dir}/* /opt/mount/"
